...
INFILE = ROOT / "data" / "raw" / "CO2_LewisCarver(1983)_370K_121-193nm.txt"
...
def load_sigma_txt(path: Path) -> pd.DataFrame:
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        for ln in f:
            ...
            # need at least wavelength + sigma
            if len(parts) < 2: 
                continue
            try:
                wl_nm = float(parts[0])
                # sigma value might be 2nd OR 3rd token (some files have an index column)
                sig   = float(parts[1])
            except:
                # try third token
                try:
                    wl_nm = float(parts[0]); sig = float(parts[2])
                except:
                    continue
            rows.append((wl_nm, sig))
    if not rows:
        raise SystemExit("No numeric rows parsed.")
    df = pd.DataFrame(rows, columns=["wl_nm", "sigma_cm2"])
    # clean + crop
    df = df[(df["wl_nm"]>0) & (df["sigma_cm2"]>0)].sort_values("wl_nm").reset_index(drop=True)
    df = df[(df["wl_nm"]>=CROP_MIN_NM) & (df["wl_nm"]<=CROP_MAX_NM)]
    if df.empty:
        raise SystemExit("After crop(121–193 nm) no data left.")
    return df
